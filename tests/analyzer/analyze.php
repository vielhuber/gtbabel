<?php
if( !file_exists('log.txt') ) { die('log missing'); }
$content = file_get_contents('log.txt');
$content = explode('##############################################', $content);
foreach(array_diff(scandir('.'), array('.', '..')) as $files__value) {
    if( strpos($files__value, '.html') === false ) { continue; }
    @unlink($files__value);
}
foreach($content as $content__key=>$content__value) {
    if( $content__key % 2 === 0 ) { continue; }
