...
[
    ...
    'auto_translation_service' => [
        [
            'provider' => 'google', // google|microsoft|deepl
            'lng' => null
            //'lng' => ['en', 'fr'] // populate if provider should act only on a specific language
        ]
    ],
...
<html lang="de">
    <body>
        ...
        <div lang="de">Inhalt auf Französisch.</div>
...
If you need to translate queries to your source language (for example in searches),\
translate the search term into the source language before actually querying your content:

```php
$s = gtbabel__($s, gtbabel_source_lng(), gtbabel_current_lng());
```

In WordPress this is done automatically for you.
...
If the mails consist of personal data, make sure to send these emails as html and properly exclude dynamic content like personal data:

```
$message = 'Hello <span class="notranslate force-tokenize">John Doe</span>';
```

Gtbabel automatically translates all mails sent in WordPress and has built in support for popular plugins like [Contact Form 7](https://de.wordpress.org/plugins/contact-form-7/) and [WooCommerce](https://de.wordpress.org/plugins/woocommerce/) to ensure the above guidelines.
