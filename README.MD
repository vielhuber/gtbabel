...
-   WordPress plugin available: configuration gui, preloading, auto translation / deletion, manual string translation, translation services included, support for plugins like [Contact Form 7](https://de.wordpress.org/plugins/contact-form-7/).
-   Works seamlessly with caching/preloading plugins.
-   Besides plain html does also handle and translate json responses.
...
If you want to influence that special tags should not be auto translated by the Google Translation API, use a special class:

`<p>Das ist das <span class="notranslate">Haus</span> vom Nikolaus</p>`

## HTML in gettext

Gtbabel preserves inline HTML-tags and leaves them inside your gettext strings. However, attributes are automatically stripped.
...
and in the translations stored like

`<a>Hallo</a> Welt!`

so that your translators are not confused. The translated version has of course the attributes back again:
...
`Das deutsche <a href="https://tld.com">Brot</a> <a href="https://tld.com">vermisse</a> ich am meisten.`

is stored in your gettext files as

`Das deutsche <a>Brot</a> <a>vermisse</a> ich am meisten.`

If your translations don't reflect the original order, they include hints like
...
The translation apis of Google or Microsoft can be costly. Try to keep track of your current usage stats. Gtbabel helps you by tracking the total amount of translated chars (see the `api_stats` option). You can also provide multiple api keys, Gtbabel then distributes the calls uniformly.
