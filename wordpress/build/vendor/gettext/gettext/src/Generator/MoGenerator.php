...
final class MoGenerator extends \ScopedGtbabel\Gettext\Generator\Generator
{
    private $includeHeaders = \false;
    public function includeHeaders(bool $includeHeaders = \true) : self
    {
    ...
    public function generateString(\ScopedGtbabel\Gettext\Translations $translations) : string
    {
        ...
        foreach ($translations as $translation) {
            ...
            if ($context = $translation->getContext()) {
                $originalString = "{$context}\4{$translation->getOriginal()}";
            } else {
        ...
        foreach ($messages as $originalString => $translation) {
            if (\is_string($translation)) {
                ...
                $originalString .= "\0{$translation->getPlural()}";
                $translationString = "{$translation->getTranslation()}\0" . \implode("\0", $translation->getPluralTranslations($pluralSize));
            } else {
    ...
    private static function hasPluralTranslations(\ScopedGtbabel\Gettext\Translation $translation) : bool
    {
