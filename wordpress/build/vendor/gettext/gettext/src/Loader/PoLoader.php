 ...
 */
final class PoLoader extends \ScopedGtbabel\Gettext\Loader\Loader
{
    public function loadString(string $string, \ScopedGtbabel\Gettext\Translations $translations = null) : \ScopedGtbabel\Gettext\Translations
    {
        ...
        while ($line !== \false) {
            ...
            $nextLine = \next($lines);
            //Multiline
            while (\substr($line, -1, 1) === '"' && $nextLine !== \false && \substr(\trim($nextLine), 0, 1) === '"') {
                $line = \substr($line, 0, -1) . \substr(\trim($nextLine), 1);
                $nextLine = \next($lines);
            }
            //End of translation
            if ($line === '') {
    ...
    public static function decode(string $value) : string
    {
        ...
        return \strtr($value, ['\\\\' => '\\', '\\a' => "\7", '\\b' => "\10", '\\t' => "\t", '\\n' => "\n", '\\v' => "\v", '\\f' => "\f", '\\r' => "\r", '\\"' => '"']);
    }
    private static function isEmpty(\ScopedGtbabel\Gettext\Translation $translation) : bool
    {
