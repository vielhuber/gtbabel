...
class Category
{
    ...
    public function __construct($cldrCategoryId, $cldrFormulaAndExamples)
    {
        ...
        if (!\in_array($matches[1], \ScopedGtbabel\Gettext\Languages\CldrData::$categories)) {
            throw new \Exception("Invalid CLDR category: '{$cldrCategoryId}'");
        }
        $this->id = $matches[1];
        ...
        switch ($this->id) {
            case \ScopedGtbabel\Gettext\Languages\CldrData::OTHER_CATEGORY:
                if ($cldrFormula !== '') {
                    throw new \Exception("The '" . \ScopedGtbabel\Gettext\Languages\CldrData::OTHER_CATEGORY . "' category should not have any formula, but it has '{$cldrFormula}'");
                }
                $this->formula = null;
                break;
            default:
                ...
                $this->formula = \ScopedGtbabel\Gettext\Languages\FormulaConverter::convertFormula($cldrFormula);
                break;
        }
    }
    /**
