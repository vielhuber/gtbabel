...
use ScopedGtbabel\Dotenv\Exception\ValidationException;
...
class Validator
{
     ...
     * @return void
     */
    public function __construct(\ScopedGtbabel\Dotenv\Repository\RepositoryInterface $repository, array $variables, $required = \true)
    {
    ...
    public function allowedRegexValues($regex)
    {
        return $this->assertCallback(function ($value) use($regex) {
            ...
            return \ScopedGtbabel\Dotenv\Regex\Regex::match($regex, $value)->success()->getOrElse(0) === 1;
    ...
    protected function assertCallback(callable $callback, $message = 'failed callback assertion')
    {
        ...
        if (\count($failing) > 0) {
            throw new \ScopedGtbabel\Dotenv\Exception\ValidationException(\sprintf('One or more environment variables failed assertions: %s.', \implode(', ', $failing)));
        }
        return $this;
    }
}
