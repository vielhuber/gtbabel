...
namespace ScopedGtbabel\Dotenv\Regex;

use ScopedGtbabel\Dotenv\Result\Error;
use ScopedGtbabel\Dotenv\Result\Result;
...
class Regex
{
    ...
    private static function pregAndWrap(callable $operation, $subject)
    {
        ...
        if (($e = \preg_last_error()) !== \PREG_NO_ERROR) {
            return \ScopedGtbabel\Dotenv\Result\Error::create(self::lookupError($e));
        }
        return \ScopedGtbabel\Dotenv\Result\Success::create($result);
    }
    /**
    ...
    {
        return \ScopedGtbabel\PhpOption\Option::fromValue(\get_defined_constants(\true))->filter(function (array $consts) {
            ...
            return \ScopedGtbabel\PhpOption\Option::fromValue(\array_search($code, $errors, \true));
