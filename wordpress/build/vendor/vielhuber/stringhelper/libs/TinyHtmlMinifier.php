...
class TinyHtmlMinifier
{
    ...
    public function __construct(array $options)
    {
        ...
        $this->skipName = '';
        $this->head = \false;
        $this->elements = ['skip' => ['code', 'pre', 'script', 'textarea'], 'inline' => ['a', 'abbr', 'acronym', 'b', 'bdo', 'big', 'br', 'cite', 'code', 'dfn', 'em', 'i', 'img', 'kbd', 'map', 'object', 'samp', 'small', 'span', 'strong', 'sub', 'sup', 'tt', 'var', 'q'], 'hard' => ['!doctype', 'body', 'html']];
    }
    // Run minifier
    ...
    private function walk(&$part)
    {
        ...
        if (!empty($tag_content)) {
            ...
            $this->build[] = ['name' => $name, 'content' => $element, 'type' => $type];
            $this->setSkip($name, $type);
            if (!empty($tag_content)) {
                $content = isset($tag_parts[1]) ? $tag_parts[1] : '';
                if ($content !== '') {
                    $this->build[] = ['content' => $this->compact($content, $name, $element), 'type' => 'content'];
                }
            }
            $this->buildHtml();
        }
    ...
    {
        if ($name == 'style') {
            $element = \str_replace([' type="text/css"', "' type='text/css'"], ['', ''], $element);
        } elseif ($name == 'script') {
            $element = \str_replace([' type="text/javascript"', " type='text/javascript'"], ['', ''], $element);
        }
        return $element;
    }
