...
namespace ScopedGtbabel\vielhuber\stringhelper;

class __
{
    public static function x($input)
    {
        if ($input === null || $input === \false || $input === '' || \is_string($input) && \trim($input) === '' || \is_array($input) && empty($input) || \is_object($input) && empty((array) $input)) {
            return \false;
        }
        if (\is_array($input) && \count($input) === 1 && \array_values($input)[0] === '') {
            return \false;
        }
        if ($input instanceof \ScopedGtbabel\Illuminate\Database\Eloquent\Relations\BelongsTo && $input->count() === 0) {
            return \false;
        }
        if ($input instanceof \ScopedGtbabel\Illuminate\Database\Eloquent\Collection && $input->count() === 0) {
            return \false;
        }
        if ($input instanceof \ScopedGtbabel\Illuminate\Support\Collection && $input->count() === 0) {
            return \false;
        }
    ...
    {
        if (self::x(@$args[0]) && $args[0] instanceof \ScopedGtbabel\Illuminate\Support\Collection) {
    ...
    public static function cookie_set($cookie_name, $cookie_value, $days = 30, $options = [])
    {
        ...
        if (__nx(@$options['expires'])) {
            $options['expires'] = \time() + 60 * 60 * 24 * $days;
        }
        ...
        if (\PHP_VERSION_ID < 70300) {
            \setcookie($cookie_name, $cookie_value, $options['expires'], $options['path'] . (__x(@$options['samesite']) ? '; samesite=' . $options['samesite'] : ''), $options['domain'], $options['secure'], $options['httponly']);
        } else {
    ...
    public static function remove_newlines($string)
    {
        $string = \preg_replace('~[\\r\\n]+~', '', $string);
        // remove nl
        $string = \str_ireplace(['<br/>', '<br />', '<br>'], '', $string);
        // remove brs
        return $string;
    }
    public static function trim_whitespace($str)
    {
        ...
        while ($str !== \trim($str) || $str !== \trim($str, ' ') || $str !== \trim($str, \html_entity_decode('&nbsp;'))) {
            $str = \trim($str);
            $str = \trim($str, ' ');
            $str = \trim($str, \html_entity_decode('&nbsp;'));
        }
        return $str;
    }
    ...
    public static function minify_html($html, $options = [])
    {
        ...
        if (\class_exists('ScopedGtbabel\\TinyHtmlMinifier') === \false) {
            require __DIR__ . '/../libs/TinyHtmlMinifier.php';
        }
        $minifier = new \ScopedGtbabel\TinyHtmlMinifier($options);
    ...
    public static function translate_google($str, $from_lng, $to_lng, $api_key)
    {
        ...
        $fix_weird_bug = \false;
        if ($fix_weird_bug === \true) {
            $str = \htmlentities($str);
        }
        $response = self::curl('https://translation.googleapis.com/language/translate/v2?key=' . $api_key, ['q' => $str, 'source' => $from_lng, 'target' => $to_lng, 'format' => 'html', 'model' => 'nmt'], 'POST');
            ...
            if ($fix_weird_bug === \true) {
                ...
                if (!empty($matches)) {
                    foreach ($matches[0] as $matches__value) {
                        $trans = \str_replace($matches__value, \trim($matches__value), $trans);
                    }
                }
    ...
    {
        $response = self::curl('https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&from=' . $from_lng . '&to=' . $to_lng . '&textType=html', [['Text' => $str]], 'POST', ['Ocp-Apim-Subscription-Key' => $api_key], \false, \true);
    ...
    public static function array_multisort($args)
    {
        return function ($a, $b) use($args) {
            $position = [\true => -1, \false => 1];
            $order = \true;
            if (\is_array($args)) {
                foreach ($args as $args__value) {
                    if ($a[$args__value[0]] != $b[$args__value[0]]) {
                        return $position[self::array_multisort_get_order($a[$args__value[0]], $b[$args__value[0]], $args__value[1])];
                    }
                }
                ...
                foreach ($args_a as $args__key => $args__value) {
                    if ($args_a[$args__key][0] != $args_b[$args__key][0]) {
                        return $position[self::array_multisort_get_order($args_a[$args__key][0], $args_b[$args__key][0], $args_a[$args__key][1])];
                    }
                }
            }
    ...
    public static function array_group_by($array, $key)
    {
        if (!\is_string($key) && !\is_int($key) && !\is_float($key) && !\is_callable($key)) {
            \trigger_error('array_group_by(): The key should be a string, an integer, or a callback', \E_USER_ERROR);
            return null;
        }
        ...
        if (\is_array($array)) {
            foreach ($array as $array__key => $array__value) {
                ...
                if (\is_array($array__value)) {
                    $new[$array__key] = self::array_map_deep($array__value, $callback, $array__key, $key_chain_this);
                } else {
    ...
    {
        if (($a instanceof \ScopedGtbabel\Illuminate\Database\Eloquent\Collection || $a instanceof \ScopedGtbabel\Illuminate\Support\Collection) && $a->count() > 0) {
            ...
            foreach ($a as $a__key => $a__value) {
                ...
                } elseif (self::nx(@$a__value) || $additional !== null && \in_array($a__value, $additional, \true)) {
                    unset($a[$a__key]);
                }
            }
    ...
    public static function remove_by_value($arr, $value)
    {
        ...
        if (\is_array($arr)) {
            foreach (\array_keys($arr, $value, \true) as $key) {
                unset($arr[$key]);
            }
            $all_numeric = \true;
            foreach ($arr as $arr__key => $arr__value) {
    ...
    public static function arr_append($array, $value, $condition = \true)
    {
    ...
    public static function arr_prepend($array, $value, $condition = \true)
    {
        if ($condition === \false || !\is_array($array)) {
            return $array;
        }
    ...
    {
        return \json_decode(\json_encode($obj), \true);
    }
        ...
        if ($strip === \true) {
            ...
            foreach ($words as $words__key => $words__value) {
                ...
                foreach ($positions as $positions__value) {
                    if ($i >= $positions__value - $strip_length && $i <= $positions__value + \mb_strlen($query) + $strip_length - 1) {
                        $strip_now = \false;
                    }
                }
                if ($strip_now === \true) {
    ...
    {
        if ($mode === 'php') {
            \header('Location: ' . self::v($url, self::baseurl()) . '', \true, $code_or_seconds);
            die;
        }
        if ($mode === 'html') {
            echo '<meta http-equiv="refresh" content="' . self::v($code_or_seconds, 0) . '; url=\'' . self::v($url, self::baseurl()) . '\'">';
            die;
        }
    }
    public static function date($date = null, $format = null, $mod = null)
    {
        ...
        if (self::nx($date) || $date === \true || $date === \false) {
            return null;
        }
        /*
        ...
        */
        if ($date !== null && $format === null && $mod === null) {
            if (self::validate_date($date) && self::validate_date_format($date)) {
                ...
                $whitelist = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'weekday', 'weekdays', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'ninth', 'tenth', 'eleventh', 'twelfth', 'next', 'last', 'previous', 'this', 'next', 'last', 'previous', 'this', 'sec', 'second', 'min', 'minute', 'hour', 'day', 'fortnight', 'forthnight', 'month', 'year', 'weeks', 'yesterday', 'midnight', 'today', 'now', 'noon', 'tomorrow', 'back of', 'front of', 'first day of', 'last day of', 'last', 'week', 'ago', 'week'];
                $found = \false;
        ...
        if (($date === null || self::validate_date($date)) && ($format === null || self::validate_date_format($format)) && ($mod === null || self::validate_date_mod($mod))) {
            // default case
        } elseif (($format === null || self::validate_date($format)) && ($date === null || self::validate_date_format($date)) && ($mod === null || self::validate_date_mod($mod))) {
            [$format, $date] = [$date, $format];
        } elseif (($date === null || self::validate_date($date)) && ($mod === null || self::validate_date_format($mod)) && ($format === null || self::validate_date_mod($format))) {
            [$format, $mod] = [$mod, $format];
        } elseif (($format === null || self::validate_date($format)) && ($mod === null || self::validate_date_format($mod)) && ($date === null || self::validate_date_mod($date))) {
            [$date, $format] = [$format, $date];
            [$format, $mod] = [$mod, $format];
        } elseif (($mod === null || self::validate_date($mod)) && ($date === null || self::validate_date_format($date)) && ($format === null || self::validate_date_mod($format))) {
            [$date, $mod] = [$mod, $date];
            [$format, $mod] = [$mod, $format];
        } elseif (($mod === null || self::validate_date($mod)) && ($format === null || self::validate_date_format($format)) && ($date === null || self::validate_date_mod($date))) {
            [$date, $mod] = [$mod, $date];
        } else {
            return null;
        }
    ...
    public static function validate_date_format($str)
    {
        ...
        $chars_needed = ['d', 'D', 'j', 'l', 'N', 'S', 'w', 'z', 'W', 'F', 'm', 'M', 'n', 't', 'L', 'o', 'Y', 'y', 'a', 'A', 'B', 'g', 'G', 'h', 'H', 'i', 's', 'u', 'v', 'e', 'I', 'O', 'P', 'T', 'Z', 'z', 'c', 'r', 'U'];
    ...
    public static function curl($url = '', $data = null, $method = null, $headers = null, $enable_cookies = \false, $send_as_json = \true, $timeout = 60, $basic_auth = null, $cookies = null, $follow_redirects = \true)
    {
        ...
        if ($follow_redirects === \true) {
            \curl_setopt($curl, \CURLOPT_FOLLOWLOCATION, \true);
        }
        if (self::x($basic_auth)) {
            \curl_setopt($curl, \CURLOPT_USERPWD, self::first_key($basic_auth) . ':' . $basic_auth[self::first_key($basic_auth)]);
        }
        if (self::x($cookies)) {
            $cookies_curl = [];
    ...
    {
        return (object) ['success' => \true, 'message' => $message];
    }
    ...
    public static function csv2array($filename, $delimiter = ';', $enclosure = '"')
    {
        ...
        $array = \array_map(function ($d) use($delimiter, $enclosure) {
            return \array_map(function ($d2) {
                if (!\mb_detect_encoding($d2, 'UTF-8', \true)) {
                    $d2 = \utf8_encode($d2);
                }
                return $d2;
    ...
    public static function sed_replace($replacements = [], $filename)
    {
        ...
        foreach ($replacements as $replacements__key => $replacements__value) {
            $command .= " -e 's/" . self::sed_escape($replacements__key) . '/' . self::sed_escape($replacements__value) . "/g'";
        }
        $command .= ' "' . $filename . '"';
        \shell_exec($command);
    }
    ...
    public static function log_begin($message = null)
    {
        ...
        $GLOBALS['performance'][] = ['message' => $message, 'time' => \microtime(\true)];
    }
    public static function log_end($message = null, $echo = \true)
    {
        if (!isset($GLOBALS['performance'])) {
            $GLOBALS['performance'] = [];
        }
        $performance_active_key = null;
        foreach (\array_reverse($GLOBALS['performance'], \true) as $performance__key => $performance__value) {
        ...
        $time = \number_format(\microtime(\true) - $GLOBALS['performance'][$performance_active_key]['time'], 5);
        if ($echo === \true) {
    ...
    {
        if ($salt === null) {
            $salt = \hash('sha256', \uniqid(\mt_rand(), \true));
        }
        // this is an unique salt per entry and directly stored within a password
        return \base64_encode(\openssl_encrypt($string, 'AES-256-CBC', \ScopedGtbabel\ENCRYPTION_KEY, 0, \str_pad(\substr($salt, 0, 16), 16, '0', \STR_PAD_LEFT))) . ':' . $salt;
    }
    public static function decrypt($string)
    {
        ...
        return \openssl_decrypt(\base64_decode($string), 'AES-256-CBC', \ScopedGtbabel\ENCRYPTION_KEY, 0, \str_pad(\substr($salt, 0, 16), 16, '0', \STR_PAD_LEFT));
    }
    public static function encrypt_poor($string)
    {
        $folder = \sys_get_temp_dir();
        if (\defined('ScopedGtbabel\\ENCRYPTION_FOLDER')) {
            $folder = \ScopedGtbabel\ENCRYPTION_FOLDER;
        }
        $folder = \rtrim($folder, '/');
        $token = \md5(\uniqid(\mt_rand(), \true));
                ...
                while (\false !== ($fileOrFolder = \readdir($handle))) {
                    if ($fileOrFolder != '.' && $fileOrFolder != '..' && !\in_array($fileOrFolder, $exclude)) {
                        if (!\is_dir($folder . '/' . $fileOrFolder)) {
                            $files[] = $fileOrFolder;
                        } elseif ($recursive === \true) {
                            $files = \array_merge($files, self::files_in_folder($folder . '/' . $fileOrFolder, $recursive));
                        }
                    }
                }
                \closedir($handle);
    ...
    public static function is_external($link)
    {
        ...
        return \strpos($link, self::baseurl()) === \false && \strpos($link, 'mailto') === \false && \strpos($link, 'tel') === \false || \strpos($link, '.pdf') !== \false;
    }
    public static function pushId()
    {
        ...
        if (!$isDuplicateTime) {
            ...
            \usleep(10);
            // additionally prevent duplicates
