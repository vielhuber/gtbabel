...
class Utils
{
    ...
    function lb($message = '')
    {
        ...
        $GLOBALS['performance'][] = ['time' => \microtime(\true), 'message' => $message];
    }
    function le()
    {
        $this->log('script ' . $GLOBALS['performance'][\count($GLOBALS['performance']) - 1]['message'] . ' execution time: ' . \number_format(\microtime(\true) - $GLOBALS['performance'][\count($GLOBALS['performance']) - 1]['time'], 5) . ' seconds');
    ...
    function getContentType($response)
    {
        ...
        if (\ScopedGtbabel\vielhuber\stringhelper\__::string_is_json($response)) {
            return 'json';
        }
