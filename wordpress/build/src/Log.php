...
class Log
{
    ...
    public $settings;
    public $host;
    function __construct(\ScopedGtbabel\vielhuber\gtbabel\Utils $utils = null, \ScopedGtbabel\vielhuber\gtbabel\Settings $settings = null, \ScopedGtbabel\vielhuber\gtbabel\Host $host = null)
    {
    ...
    {
        return \rtrim($this->utils->getDocRoot(), '/') . '/' . \trim($this->settings->get('log_folder'), '/');
    }
    ...
    function discoveryLogGet($since_time = null, $url = null, $slim_output = \true)
    {
        $strings = [];
        ...
        foreach ($data as $data__value) {
            ...
            $strings[] = ['string' => $line_parts[1], 'context' => $line_parts[2], 'url' => $line_parts[0], 'lng' => $line_parts[3], 'date' => $line_parts[4], 'order' => \count($strings) + 1];
        }
        ...
        if ($slim_output === \true) {
            ...
            $strings = \ScopedGtbabel\vielhuber\stringhelper\__::array_unique($strings);
    ...
    function discoveryLogAdd($url, $str, $context, $lng)
    {
        ...
        $data[] = $url . "\t" . $str . "\t" . $context . "\t" . $lng . "\t" . \microtime(\true);
        $data = \implode(\PHP_EOL, $data);
        \file_put_contents($filename, $data);
