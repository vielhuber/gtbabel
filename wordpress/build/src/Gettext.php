...
use ScopedGtbabel\Gettext\Generator\PoGenerator;
use ScopedGtbabel\Gettext\Generator\MoGenerator;
use ScopedGtbabel\Gettext\Loader\PoLoader;
use ScopedGtbabel\Gettext\Loader\MoLoader;
...
class Gettext
{
    ...
    public $log;
    public $publish;
    function __construct(\ScopedGtbabel\vielhuber\gtbabel\Utils $utils = null, \ScopedGtbabel\vielhuber\gtbabel\Host $host = null, \ScopedGtbabel\vielhuber\gtbabel\Settings $settings = null, \ScopedGtbabel\vielhuber\gtbabel\Tags $tags = null, \ScopedGtbabel\vielhuber\gtbabel\Log $log = null, \ScopedGtbabel\vielhuber\gtbabel\Publish $publish = null)
    {
        ...
        $this->publish = $publish ?: new \ScopedGtbabel\vielhuber\gtbabel\Publish();
    }
    function preloadGettextInCache()
    {
        ...
        $poLoader = new \ScopedGtbabel\Gettext\Loader\PoLoader();
        // pot
        $filename = $this->getLngFilename('pot', '_template');
        $this->gettext_save_counter['pot'] = \false;
        if (!\file_exists($filename)) {
            $this->gettext_pot = \ScopedGtbabel\Gettext\Translations::create('gtbabel');
        } else {
        ...
        foreach ($this->settings->getSelectedLanguageCodesWithoutSource() as $languages__value) {
            ...
            if (!\file_exists($this->getLngFilename('po', $languages__value))) {
                $this->gettext[$languages__value] = \ScopedGtbabel\Gettext\Translations::create('gtbabel');
            } else {
    ...
    function generateGettextFiles()
    {
        ...
        $poGenerator = new \ScopedGtbabel\Gettext\Generator\PoGenerator();
        $moGenerator = new \ScopedGtbabel\Gettext\Generator\MoGenerator();
        if ($this->gettext_save_counter['pot'] === \true) {
    ...
    function convertPoToMo($filename)
    {
        ...
        $loader = new \ScopedGtbabel\Gettext\Loader\PoLoader();
        $translations = $loader->loadFile($filename);
        $generator = new \ScopedGtbabel\Gettext\Generator\MoGenerator();
    ...
    function getExistingTranslationFromCache($str, $lng, $context = null)
    {
        ...
        if ($str === '' || $str === null || $this->gettext_cache[$lng] === null || !\array_key_exists($context ?? '', $this->gettext_cache[$lng]) || !\array_key_exists($str, $this->gettext_cache[$lng][$context ?? '']) || $this->gettext_cache[$lng][$context ?? ''][$str] === '') {
            return \false;
        }
    ...
    {
        if ($str === '' || $str === null || $this->gettext_cache_reverse[$lng] === null || !\array_key_exists($context ?? '', $this->gettext_cache_reverse[$lng]) || !\array_key_exists($str, $this->gettext_cache_reverse[$lng][$context ?? '']) || $this->gettext_cache_reverse[$lng][$context ?? ''][$str] === '') {
            return \false;
        }
    ...
    function getAllTranslationsFromFiles()
    {
        $data = [];
        $poLoader = new \ScopedGtbabel\Gettext\Loader\PoLoader();
        $moLoader = new \ScopedGtbabel\Gettext\Loader\MoLoader();
        ...
        foreach ($pot->getTranslations() as $gettext__value) {
            ...
            $data[$this->getTranslationHash($gettext__value)] = ['orig' => $gettext__value->getOriginal(), 'context' => $gettext__value->getContext() ?? '', 'shared' => $shared, 'translations' => []];
        }
        foreach ($this->settings->getSelectedLanguageCodesWithoutSource() as $languages__value) {
            ...
            foreach ($mo->getTranslations() as $gettext__value) {
                $data[$this->getTranslationHash($gettext__value)]['translations'][$languages__value] = $gettext__value->getTranslation();
            }
        }
    ...
    function getSharedValueFromTranslation($gettext)
    {
        ...
        foreach ($gettext->getExtractedComments() as $comments__value) {
            ...
            $shared = \trim(\explode(':', $comments__value)[1]) == '1' ? \true : \false;
            break;
        }
        return $shared;
    }
    ...
    function autoEditSharedValueFromFiles($strings, $not_in_urls)
    {
        ...
        foreach ($strings as $strings__value) {
            ...
            foreach ($discovery_log as $discovery_log__value) {
                if ($discovery_log__value['string'] == $strings__value['string'] && $discovery_log__value['context'] == $strings__value['context'] && !\in_array($discovery_log__value['url'], $not_in_urls)) {
                    $shared = \true;
                    break;
                }
            }
        ...
        foreach ($pot->getTranslations() as $gettext__value) {
            ...
            $gettext__value->getExtractedComments()->add('shared: ' . ($data[$hash] === \true ? '1' : '0'));
            $success = \true;
        }
        if ($success === \true) {
            $poGenerator->generateFile($pot, $this->getLngFilename('pot', '_template'));
    ...
    {
        $translation = \ScopedGtbabel\Gettext\Translation::create($context, $str);
        $translation->translate('');
        if ($this->settings->get('auto_add_added_date_to_gettext') === \true) {
        ...
        $this->gettext_save_counter['pot'] = \true;
    }
    function addTranslationToPoFileAndToCache($orig, $trans, $lng, $context = null, $comment = null)
    {
        ...
        $translation = \ScopedGtbabel\Gettext\Translation::create($context, $orig);
        ...
        $this->gettext_save_counter['po'][$lng] = \true;
    }
    function getLngFolder()
    {
        return $this->utils->getDocRoot() . '/' . \trim($this->settings->get('lng_folder'), '/');
    }
    ...
    {
        return $this->host->getCurrentHost() . '/' . \trim($this->settings->get('lng_folder'), '/');
    }
    ...
    {
        $files = \glob($this->getLngFolder() . '/*');
        // get all file names
        foreach ($files as $files__value) {
            if (\is_file($files__value)) {
                if (\mb_strpos($files__value, '.pot') !== \false || \mb_strpos($files__value, '.po') !== \false || \mb_strpos($files__value, '.mo') !== \false) {
                    @\unlink($files__value);
                }
            }
        }
    ...
    function createLngFolderIfNotExists()
    {
        if (!\is_dir($this->getLngFolder())) {
            \mkdir($this->getLngFolder(), 0777, \true);
        }
    }
    ...
    function getLanguagePickerData()
    {
        ...
        foreach ($this->settings->getSelectedLanguages() as $languages__key => $languages__value) {
            ...
            if ($this->publish->isActive() && $this->publish->isPrevented($this->host->getCurrentUrl(), $languages__key)) {
                continue;
            }
            $data[] = ['code' => $languages__key, 'label' => $languages__value, 'url' => $url, 'active' => \rtrim($url, '/') === \rtrim($this->host->getCurrentUrlWithArgs(), '/')];
        }
        return $data;
    }
    ...
    function prepareTranslationAndAddDynamicallyIfNeeded($orig, $lng, $context = null)
    {
        ...
        if ($context === 'slug') {
            $trans = $this->getTranslationOfLinkHrefAndAddDynamicallyIfNeeded($orig, $lng, \true);
            if ($trans === null) {
                return $orig;
            }
            return $trans;
    ...
    {
        if ($link === null || \trim($link) === '') {
            return $link;
        }
        if (\mb_strpos(\trim($link, '/'), '#') === 0) {
            return $link;
        }
        if (\mb_strpos(\trim($link, '/'), '?') === 0) {
            return $link;
        ...
        $link = \str_replace([$this->host->getCurrentHost() . '/' . $this->settings->getSourceLanguageCode(), $this->host->getCurrentHost()], '', $link);
        if ($translate === \true) {
    ...
    function autoTranslateString($orig, $to_lng, $context = null, $from_lng = null)
    {
        ...
        $trans = null;
        if ($this->settings->get('auto_translation') === \true) {
            if ($this->settings->get('auto_translation_service') === 'google') {
                ...
                $trans = \ScopedGtbabel\vielhuber\stringhelper\__::translate_google($orig, $from_lng, $to_lng, $api_key);
                ...
                $trans = \ScopedGtbabel\vielhuber\stringhelper\__::translate_microsoft($orig, $from_lng, $to_lng, $api_key);
        ...
        if ($this->settings->get('debug_translations') === \true) {
            if ($context !== 'slug') {
                $trans = '%|%' . $trans . '%|%';
            }
        }
    ...
    function removeLineBreaks($orig)
    {
        ...
        $str = \trim($str);
        $str = \str_replace(['&#13;', "\r"], '', $str);
        // replace nasty carriage returns \r
        $str = \preg_replace('/[\\t]+/', ' ', $str);
        // replace multiple tab spaces with one tab space
        $parts = \explode(\PHP_EOL, $str);
        foreach ($parts as $parts__key => $parts__value) {
            $parts__value = \trim($parts__value);
            if ($parts__value == '') {
    ...
    function reintroduceLineBreaks($str, $orig_withoutlb, $orig_with_lb)
    {
        ...
        while (\mb_substr($orig_with_lb, $pos_lb_end, 1) !== \mb_substr($orig_withoutlb, \mb_strlen($orig_withoutlb) - 1, 1)) {
            $pos_lb_end--;
        }
    ...
    {
        if ($str === null || $str === \true || $str === \false || $str === '') {
            return \true;
        }
        $str = \trim($str);
        $str = \trim($str, '"');
        ...
        foreach ($this->settings->getSelectedLanguageCodes() as $languages__value) {
            if ($languages__value === \trim(\mb_strtolower($str))) {
                return \true;
            }
        }
        if ($context === 'slug') {
            if (\mb_strpos(\trim($str, '/'), '#') === 0) {
                return \true;
            }
            if (\mb_strpos(\trim($str, '/'), '?') === 0) {
                return \true;
        ...
        if (\mb_strpos($str, '(') === 0 && \mb_strrpos($str, ')') === \mb_strlen($str) - 1 && \mb_strpos($str, '=') !== \false) {
            return \true;
        }
    ...
    function getUrlTranslationInLanguage($lng, $url = null)
    {
        ...
        return \trim(\trim($this->host->getCurrentHost(), '/') . '/' . \trim($this->getPathTranslationInLanguage($lng, \false, $path), '/'), '/') . (\mb_strpos($path, '?') === \false ? '/' : '');
    }
    function getTranslationInForeignLng($str, $to_lng, $from_lng = null, $context = null)
    {
        ...
        if ($from_lng === $this->settings->getSourceLanguageCode()) {
            ...
            $str_in_source_lng = $this->getExistingTranslationReverseFromCache($str, $from_lng, $context);
            // str in source lng
        }
    ...
    function getTranslationInForeignLngAndAddDynamicallyIfNeeded($str, $to_lng = null, $from_lng = null, $context = null)
    {
        ...
        if ($trans === \false) {
            if ($from_lng === $this->settings->getSourceLanguageCode()) {
                ...
                $str_in_source = $this->autoTranslateString($str, $this->settings->getSourceLanguageCode(), $context, $from_lng);
            }
    ...
    function getPathTranslationInLanguage($lng, $always_remove_prefix = \false, $path = null)
    {
        ...
        // prefix
        if ($always_remove_prefix === \true || $this->settings->getSourceLanguageCode() === $lng && $this->settings->get('prefix_source_lng') === \false) {
