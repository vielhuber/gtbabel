...
class Dom
{
    ...
    public $host;
    public $settings;
    function __construct(\ScopedGtbabel\vielhuber\gtbabel\Utils $utils = null, \ScopedGtbabel\vielhuber\gtbabel\Gettext $gettext = null, \ScopedGtbabel\vielhuber\gtbabel\Host $host = null, \ScopedGtbabel\vielhuber\gtbabel\Settings $settings = null)
    {
        $this->utils = $utils ?: new \ScopedGtbabel\vielhuber\gtbabel\Utils();
        $this->gettext = $gettext ?: new \ScopedGtbabel\vielhuber\gtbabel\Gettext();
        ...
        if ($this->settings->get('exclude_dom') !== null) {
            foreach ($this->settings->get('exclude_dom') as $exclude__value) {
                ...
                foreach ($nodes as $nodes__value) {
                    $this->excluded_nodes[$this->getIdOfNode($nodes__value)] = \true;
                    foreach ($this->getChildrenOfNode($nodes__value) as $nodes__value__value) {
                        $this->excluded_nodes[$this->getIdOfNode($nodes__value__value)] = \true;
                    }
                }
            }
        }
    ...
    function modifyTextNodes()
    {
        ...
        foreach ($textnodes as $textnodes__value) {
            ...
            foreach ($children as $children__value) {
                $to_delete[$this->getIdOfNode($children__value)] = \true;
            }
        }
        ...
        foreach ($groups as $groups__value) {
            ...
            $translatedText = $this->gettext->prepareTranslationAndAddDynamicallyIfNeeded($originalText, $this->gettext->getCurrentLanguageCode(), null);
    ...
    function modifyTagNodes()
    {
        ...
        $include = [];
        if ($this->settings->get('translate_default_tag_nodes') === \true) {
            $include = \array_merge($include, [['selector' => 'a', 'attribute' => 'href', 'context' => 'slug'], ['selector' => 'form', 'attribute' => 'action', 'context' => 'slug'], ['selector' => 'img', 'attribute' => 'alt', 'context' => null], ['selector' => 'input', 'attribute' => 'placeholder', 'context' => null], ['selector' => 'head title', 'attribute' => null, 'context' => 'title'], ['selector' => 'head meta[name="description"]', 'attribute' => 'content', 'context' => 'description']]);
        }
            ...
            if (!empty($nodes)) {
                foreach ($nodes as $nodes__value) {
                    ...
                    if ($value != '') {
                        ...
                        if ($this->gettext->sourceLngIsCurrentLng()) {
                            if ($context === 'slug') {
                                $trans = $this->gettext->getTranslationOfLinkHrefAndAddDynamicallyIfNeeded($value, $this->gettext->getCurrentLanguageCode(), \false);
                                if ($trans === null) {
                                    continue;
                                }
                            } else {
                            ...
                            $trans = $this->gettext->prepareTranslationAndAddDynamicallyIfNeeded($value, $this->gettext->getCurrentLanguageCode(), $context);
                        }
                        if (@$include__value['attribute'] != '') {
    ...
    function finishDomDocument($htmlOriginal)
    {
        ...
        if (\mb_stripos($htmlOriginal, '<!DOCTYPE') !== 0 && \mb_stripos($htmlOriginal, '<html') !== 0 && \mb_stripos($htmlModified, '<body>') !== \false && \mb_stripos($htmlModified, '</body>') !== \false) {
            ...
            if (\mb_strpos($parts__value, '.') !== \false) {
                ...
                foreach ($parts__value_parts as $parts__value_parts__key => $parts__value_parts__value) {
                    ...
                    if ($parts__value_parts__key > 0) {
                        $parts__value_parts[$parts__value_parts__key] = '[contains(concat(" ", normalize-space(@class), " "), " ' . $parts__value_parts__value . ' ")]';
                    }
                }
            ...
            if ($result) {
                ...
                if ($result) {
                    ...
                    foreach ($import->childNodes as $child) {
                        $importedNode = $node->ownerDocument->importNode($child, \true);
    ...
    function getNearestLogicalGroup($node)
    {
        ...
        if (!\array_key_exists($this->getIdOfNode($parent), $this->group_cache)) {
            ...
            if (\in_array($this->getIdOfNode($parent), $this->force_tokenize)) {
                $this->group_cache[$this->getIdOfNode($parent)] = \true;
            }
            ...
            foreach ($this->getChildrenOfNode($parent) as $nodes__value) {
                ...
                if (!($this->isTextNode($nodes__value) || $this->isInnerTagNode($nodes__value) && $this->getChildrenCountRecursivelyOfNodeTagsOnly($nodes__value) <= 2)) {
                    $this->group_cache[$this->getIdOfNode($parent)] = \true;
                    break;
                }
            }
        }
        if ($this->group_cache[$this->getIdOfNode($parent)] === \true) {
            return $node;
        }
        return $parent;
    }
        ...
        if (!empty($translated_strings)) {
            ...
            foreach ($translated_strings as $translated_strings__value) {
                if (!\is_array($translated_strings__value)) {
                    ...
                    $trans = $this->gettext->getTranslationInForeignLngAndAddDynamicallyIfNeeded($translated_strings__value);
                } else {
                    $context = $translated_strings__value[1];
                    $orig = $translated_strings__value[0];
                    $trans = $this->gettext->getTranslationInForeignLngAndAddDynamicallyIfNeeded($translated_strings__value[0], null, null, $translated_strings__value[1]);
                }
            ...
            echo 'var translated_strings = JSON.parse(\'' . \json_encode($translated_strings_json, \JSON_HEX_APOS) . '\');';
    ...
    function traverseJson(&$json)
    {
        if (\is_array($json) || \is_object($json) || $json instanceof \Traversable) {
            foreach ($json as $json__key => &$json__value) {
                ...
                } elseif (\is_string($json__value) && \in_array($json__key, ['message'], \true)) {
                    $json__value = $this->gettext->prepareTranslationAndAddDynamicallyIfNeeded($json__value, $this->gettext->getCurrentLanguageCode(), null);
                }
            }
        }
    }
