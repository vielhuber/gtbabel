...
class Host
{
    ...
    public $settings;
    function __construct(\ScopedGtbabel\vielhuber\gtbabel\Settings $settings = null)
    {
        $this->settings = $settings ?: new \ScopedGtbabel\vielhuber\gtbabel\Settings();
    }
    function setup()
    {
        $this->original_path = \parse_url($_SERVER['REQUEST_URI'], \PHP_URL_PATH);
        // store without get parameters
        ...
        $this->original_url = 'http' . (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . \parse_url($_SERVER['REQUEST_URI'], \PHP_URL_PATH);
        $this->original_url_with_args = 'http' . (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
    ...
    function urlIsExcluded($url)
    {
        if ($this->settings->get('exclude_urls') !== null && \is_array($this->settings->get('exclude_urls'))) {
            foreach ($this->settings->get('exclude_urls') as $exclude__value) {
                if (\mb_strpos(\trim($url, '/'), \trim($exclude__value, '/')) !== \false) {
                    return \true;
                }
            }
        }
